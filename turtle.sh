...
read turtlelan
if [[ $turtlelan == '' ]]; then 
turtlelan=enp0s20f0u2 # Interface of ethernet cable directly connected to Turtle
fi

echo -n "Interface between PC and Internet [wlan0]: "
read turtlewan
if [[ $turtlewan == '' ]]; then 
turtlewan=wlp1s0 #i.e. wlan0 for wifi, ppp0 for 3g modem/dialup, eth0 for lan
fi
